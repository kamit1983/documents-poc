<template>
  ...
  const authenticateFingerprint = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:3000/api/fingerprint/generate-authentication-options', {
        method: 'POST',
      ...
      const verificationResp = await fetch('http://localhost:3000/api/fingerprint/verify-authentication', {
        method: 'POST',
        headers: {
